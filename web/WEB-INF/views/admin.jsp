    ...
    <div class="card">
      <table class="table" id="profTable">
        ...
        <c:forEach var="p" items="${professors}">
          <tr data-id="${p.profId}" data-name="${p.fullName}" data-dept="${p.deptId}">
            <td>${p.profId}</td>
            <td>${p.fullName}</td>
            <td>
              <c:forEach var="d" items="${departments}">
                <c:if test="${d.id == p.deptId}">
            ...
            <td class="actions">
              <button class="btn ghost btnEditProf"
                      data-id="${p.profId}" data-name="${p.fullName}" data-dept="${p.deptId}">Edit</button>
      ...
      <c:if test="${empty professors}">
        <div class="empty">No professors yet.</div>
      </c:if>
    </div>
  </section>
  ...
  <section id="tab-ratings" class="tab-panel">
    ...
    <div class="card">
      <table class="table" id="ratingTable">
        <thead>
          <tr>
            <th>ID</th><th>Prof</th><th>Subject</th><th>Score</th>
            <th>Comment</th><th>Year</th><th>Term</th><th>Created</th><th class="col-actions">Actions</th>
          </tr>
        </thead>
        <tbody>
        <c:forEach var="r" items="${ratings}">
          <tr data-id="${r.ratingId}" data-score="${r.score}"
              data-year="${r.academicYear}" data-term="${r.term != null ? r.term.dbValue : ''}">
            <td>${r.ratingId}</td>
            <td>${r.profId}</td>
            <td>${r.subjectId}</td>
            <td><span class="star">${r.score}</span></td>
            <td class="truncate" title="${r.comment}">${r.comment}</td>
            <td>${r.academicYear}</td>
            <td><c:out value="${r.term != null ? r.term.dbValue : ''}"/></td>
            <td>${r.createdAt}</td>
            <td class="actions">
              <form method="post" action="${pageContext.request.contextPath}/do.admin.delete" class="inline confirmDelete">
...
<div id="profModal" class="modal hidden" role="dialog" aria-modal="true" aria-labelledby="profModalTitle">
  <form method="post" action="${pageContext.request.contextPath}/do.admin.professor.save" class="modal-card" id="profForm">
    ...
    <div class="modal-body">
      ...
      <label class="field"><span>Name</span><input type="text" name="name" id="profName" required></label>
      <label class="field">
