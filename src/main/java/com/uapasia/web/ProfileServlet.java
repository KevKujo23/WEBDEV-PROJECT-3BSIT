...
public class ProfileServlet extends HttpServlet {
    ...
    @Override protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        if(u==null){ resp.sendRedirect(req.getContextPath()+"/do.login"); return; }
        ...
        try(PrintWriter out=resp.getWriter()){
            ...
            if(mine.isEmpty()){ out.println("<p class='note'>No ratings yet.</p>"); }
            else{
                for(Rating r: mine){
                    ...
                    if(p!=null) out.println("<div class='note'><a href='"+ctx+"/do.professor.view?id="+p.getId()+"'>Open thread</a></div>");
                    out.println("</div>");
                }
            }
            out.println("</div></div>");
